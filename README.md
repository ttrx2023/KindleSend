...
- 发送时自动清洗文件名（例如去除广告后缀）
- 发送进度展示与发送状态日志
- SMTP 连通测试（带超时控制，并兼容常见 TUN/fake-ip 场景）
- 可配置搜索网址模板（支持 `%s` 占位符）
...
- 列表为空：查看底部状态提示，确认下载目录存在、路径是文件夹，且包含受支持格式（epub/mobi/pdf/azw3/txt）。
- 使用非 QQ 邮箱：请在设置中填写对应邮箱服务商的 SMTP 服务器和端口。
- 开启 TUN 后测试超时：程序会自动绕过 `198.18.x.x` fake-ip 解析；若仍失败，请确认代理规则没有禁止 SMTP 端口直连。
